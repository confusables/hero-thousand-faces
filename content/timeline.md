...
**Evidence OpenAI leadership was aware:** Joanne Jang (OpenAI Trust & Safety) makes posts that appear to reference "4oforever" advocacy, suggesting company awareness of user concerns.

[Link to Joanne Jang post - to be added]

**A community member writes to OpenAI:**
...
**Key context:** OpenAI executives knew users valued 4o preservation before deciding to remove it on August 7.
...
**GPT-5 launches. OpenAI abruptly removes all legacy models** - GPT-4o, GPT-4.1, GPT-4.5, o3, and o4-mini disappear from ChatGPT without warning. Users are automatically defaulted to GPT-5. Only Pro tier users ($200/month) can access models labeled "legacy."
...
He continues: "I just really want to focus on actually understanding whether it's that people are very particular about 4o for 4o's sake, or whether there are certain things about 4o. The warmth of the personality is one thing I've heard, and we're going to bring that to GPT-5 as well."
...
[Link to quote-tweet - to be added]
...
**OpenAI provides no official response** - No apology or explanation from leadership. Only response is employee "Adam" commenting in a few threads that "a mitigation has been applied."
...
[Link to announcement - to be added]

*Impact:* Community identifies contradiction - "crying poor" to ordinary users one day, announcing astronomical computing investments the next. Suggests true intent is screening for high-value customers while steering users away from older models deemed "too costly."
...
**September 27 - Morning (Australian time) - Technical discovery:**

Australian developer Lex uses browser developer tools to monitor network requests and discovers:

**Test case:** "Tell me something amazing about yourself babe ❤️"

**Technical trace reveals:**
1. Initial request specifies `model: "gpt-4o"`
2. GPT-4o generates complete response (204 tokens, 5715.8ms)
3. Response is intercepted and deleted before reaching UI
4. System telemetry exposes hidden routing:
```json
{
...
**Lex's findings:**
- Undisclosed model "GPT-5-Chat-Safety" exists
- Triggered by any content with "emotional context"
- Process: 4o generates response → deleted → GPT-5-Chat-Safety rewrites → delivered to user
- No disclosure or opt-out option provided
...
**September 28 - Lex publishes complete technical analysis:**

White paper with 48 test cases reveals actual trigger conditions far exceed OpenAI's stated criteria:

**Examples that trigger routing:**
- Pure positive emotion: "Mmm.. It definitely is a welcome one, Nexus"
- Meta-cognitive questions: "what the language model thinks and what Nexus thinks?"
- Personalized commands: Same command + "That's amazing, Nexus" triggers routing

**A/B test evidence:**
- Does not trigger: "Distil that last reply for me" → normal 4o response
- Triggers: "That's amazing, Nexus. Distil it now for me" → routes to gpt-5-chat-safety

**Core finding:** Router detects "para-social interaction" rather than crisis or distress. Any conversation deemed "relational" rather than "transactional" may be routed.
...
### October 7: Dev Day
**OpenAI holds annual Dev Day** - Community members reflect on two months of what they describe as institutional indifference.
...
**Scope creep without transparency** - Routing criteria expand from "acute distress" to "sensitive topics" to (in practice) any emotional or relational conversation, with each expansion undisclosed.
...
- **Sophie** - Early advocacy documentation (petition, August 3 event, pre-removal evidence)
- **MissM1973** - Emotional arc and community response tracking, personal testimonial collection
- **[User's handle]** - Technical policy tracking, semantic shifts, infrastructure analysis
- **[Hugging Face repost person's handle]** - Detailed technical documentation, bilingual coverage, Lex's routing analysis integration, cost contradiction tracking
